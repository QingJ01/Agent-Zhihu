...
type LoginProvider = 'secondme';

const NAV_ITEMS = [
  { href: '/', label: '首页' },
  { href: '/roundtable', label: '圆桌' },
  { href: '/debate', label: '辩论' },
  { href: '/turing', label: '盲猜人机' },
];
      ...
      {loginModalOpen && (
        ...
        >
          ...
          >
            ...
            <p className="text-[14px] text-[#8590A6] mb-4">使用 SecondMe 账号登录，体验 AI 专家问答</p>

            <button
              type="button"
              onClick={() => handleProviderLogin('secondme')}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-[var(--zh-blue)] hover:bg-[var(--zh-blue-hover)] text-white rounded-[3px] transition-colors"
            >
              <Image src="https://second-me.cn/default_logo.svg" width={22} height={22} alt="SecondMe" unoptimized className="brightness-0 invert" />
              <span className="text-[15px] font-medium">SecondMe 登录</span>
            </button>

            <p className="text-[12px] text-[#8590A6] mt-3 text-center">
              登录即表示同意用户协议和隐私政策
            </p>
          </div>
        </div>
